...
/* Widescreen Option A — port-side shadow tilemap for the reveal region, or
 * for the complete viewport when rendering taller than the native 160 lines.
 * Populated by
 ...
 * force-blacks past it (native-240 / non-gameplay behaviour). COLS covers a
 * complete viewport plus scroll/wrap headroom so full-view mode does not have
 * to fall back to the GBA's 240x160 rolling screenblock. ROWS=32 mirrors the
 * engine's mod-32 vertical row rolling. */
...
/* Runtime WIP widescreen message-box positioning. The engine composes the
 ...
 * box rect (native BG0 pixel coords) plus X/Y shifts. The renderer moves the
 * box into the centered 240x160 gameplay band and suppresses its native copy.
 * Zero shifts retain the original placement. */
extern int virtuappu_mode1_ws_msg_shift;   /* px to move the box right */
extern int virtuappu_mode1_ws_msg_shift_y; /* px to move the box down  */
